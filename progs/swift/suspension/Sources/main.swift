...
struct AsyncProgram {
    ...
    static func asyncInt() async -> Int {
        ...
        let me = counter
        counter += 1
        let until = Int.random(in: 0...50000)
    ...
    // initialized by the expression on the right-hand side of the =. However,
    // it differs in that the initializer expression is evaluated in a separate,
    ...
    // in the same manner as task group child-tasks do. It is a future direction to
    // allow customizing which executor these should be executing on. On normal
    // completion, the child task will initialize the variables in the async let.
