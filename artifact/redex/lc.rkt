...
(define -->lc/core
  (reduction-relation
   ...
   [--> (σ_0 (in-hole E (letrec ([x v] ...) e_body)))
        (σ_1 (in-hole E e_subst))

        (where/error (x_fresh ...) (gensyms (σ_0 v ... e_body) (x ...)))
        (where/error (v_subst ...) ((substitute* v (x x_fresh) ...) ...))
        (where/error σ_1 (ext σ_0 (x_fresh v_subst) ...))
        (where/error e_subst (substitute* e_body (x x_fresh) ...))
        "letrec"]
