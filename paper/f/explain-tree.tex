...
\newcommand{\cfgRoot}{\begin{tabular}{@{}l@{}}
  ...
  $\quad \mathbfsf{print}\ \text{"A"};\\\quad\mathbfsf{await}\ \mathbfsf{sys\_io}(2, ());\\\quad\mathbfsf{print}\ \text{"B"}$\\
  \fgap
  $\mathit{process\_detach} \triangleq \mathbfsf{async}\ \mathbfsf{fn}\ ()\ \to\\\quad\mathbfsf{spawn}\ (\mathit{work}())$\\
  \fgap
  $\mathit{ex2} \triangleq \mathbfsf{async}\ \mathbfsf{fn}\ ()\ \to$\\
  $\quad \mathbfsf{await}\ (\mathit{process\_detach}());\\\quad \mathbfsf{await}\ \mathbfsf{sys\_io}(1, ());\\\quad \mathbfsf{print}\ \text{"C"}$\\
  \fgap
  $\mathbfsf{sys\_block}(\mathit{ex2}())$\\
  \fgap
...
\newcommand{\cfgQueued}{\begin{tabular}{@{}l@{}}
  ...
  \hphantom{$Q$: }$\mathbfsf{await}\ \mathbfsf{sys\_io}(2, ());$\\
...
\newcommand{\cfgQueuedSwift}{\begin{tabular}{@{}l@{}}
  ...
  \hphantom{$C[\,$}\dimpro{$\mathbfsf{catch\ fn}\ \mathit{\_}\ \to\ ()$}
  $\,]$\\
  \fgap
  $Q$: $\mathbfsf{print}\ \text{"A"};$\\
  \hphantom{$Q$: }$\mathbfsf{await}\ \mathbfsf{sys\_io}(2, ());$\\
...
\newcommand{\cfgSettled}[1]{\begin{tabular}{@{}l@{}}
  $();$\\
  $\mathbfsf{await}\ \mathbfsf{sys\_io}(1, ());$\\
...
\noindent
% {\tiny where $C \triangleq [\cdot];\ \mathbfsf{await}\ \mathbfsf{sys\_io}(1, 0);\ \mathbfsf{print}\ \text{"C"}$ --- the rest of $\mathit{ex2}$; $\mathbfsf{sys\_block}$ is elided until \semrule{Block-Done} applies}\par\smallskip
