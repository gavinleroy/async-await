...
%
In Swift, when a task is cancelled, a flag is set indicating that the task is cancelled. Functions can then opt-in to raising a cancellation exception when this flag is set. It is encouraged to indicate a cancelled task by raising a \code|CancellationException|, which the standard library \code|Task.sleep| does.
