...
\paragraph{Design rationale.} For languages with \dyn \extent, the standard approach is to use cancelled destruction. The idea is that responsible async code should await on a task in the same scope in which it was spawned. If a task is still executing after its parent scope exits, then this it is probably unintented to let that task continue, and it could create surprising behavior to wait until that task is completed. However, this approach makes less sense in Trio, whose nurseries do not expose any concept of a task handle, so it is not trivial to directly await the result of a spawned task.
